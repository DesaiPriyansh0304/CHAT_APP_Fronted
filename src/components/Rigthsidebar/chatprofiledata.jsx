...
const RightProfilePanel = ({ userData, isGroup, onClose }) => {
  ...
  const getFilteredFiles = () => {
    const allFiles = messages.flatMap((msg) =>
      (Array.isArray(msg.content) ? msg.content : [msg.content]).filter(
        (c) => typeof c === 'string' && c.trim() !== '' && !isImage(c)
      )
    );
  ...
  return (
    <div className="absolute top-0 right-0 h-full w-100 bg-white dark:bg-gray-900 shadow-lg border-l border-gray-200 dark:border-gray-700 z-50 overflow-y-auto">
      ...
      <div className="p-2 text-right">
        <button
          onClick={onClose}
          className="text-gray-500 dark:text-gray-300 hover:text-black dark:hover:text-white text-xl"
        >
          ✕
        </button>
      </div>
      ...
      <div className="p-4 text-gray-800 dark:text-gray-100">
        ...
        <div className="w-full border border-amber-300 mt-3 rounded-md dark:border-amber-700">
          <ul
            className="flex px-1.5 py-1.5 list-none bg-slate-100 dark:bg-gray-800 rounded-md"
            role="list"
          >
            {tabs.map((item) => (
              <li key={item.label} className="flex-auto text-center">
                <button
                  ...
                  className={`flex items-center justify-center w-full px-0 py-2 text-sm rounded-lg transition-all cursor-pointer ${
                ...
                <div className="space-y-2">
                  {getFilteredFiles().length > 0 ? (
                    getFilteredFiles().map((file, i) => {
                      ...
                      const fileType =
                        fileTypes.find((f) => f.extensions.includes(ext)) ||
                        fileTypes.find((f) => f.type === 'Other');

                      return (
                        <div
                          key={i}
                          className="bg-gray-100 dark:bg-gray-800 p-3 rounded-lg flex items-center justify-between"
                        >
