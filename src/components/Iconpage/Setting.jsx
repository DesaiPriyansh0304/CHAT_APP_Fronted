          ...
          <div className='relative inline-block'>
            <div className='flex gap-2 items-center cursor-pointer ' onClick={toggleAvailbleMenu}>
              ...
              <span className='text-gray-600'>
                {showAvailble ? <ChevronUp className="w-4 h-4 mt-1 dark:text-[var(--text-color1)]" /> : <ChevronDown className="w-4 h-4 mt-1 dark:text-[var(--text-color)]" />}
              </span>
            </div>
            {showAvailble && (
        ...
        <div className="w-full bg-gray-200 dark:bg-[var(--text-color)] rounded">
          <div className="flex items-center justify-between cursor-pointer px-4 py-2.5 mt-3.5"
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-700 dark:text-[var(--text-color3)] font-medium">Personal Info</span>
            </div>
          ...
          {activeSection === "personalinfo" && (
            <div className='bg-white dark:bg-[var(--text-color3)]'>
              <div className="mt-2 space-y-3 text-sm text-gray-800 px-4 py-2.5 max-h-[37vh] overflow-auto">
                ...
                <div><p className="text-gray-500 dark:text-[var(--text-color)] mt-6">Email</p><p>{user?.email}</p></div>
                <div><p className="text-gray-500 dark:text-[var(--text-color)] mt-6">Mobile No</p><p>{user?.mobile}</p></div>
                <div><p className="text-gray-500 dark:text-[var(--text-color)] mt-6">DOB</p><p>{user?.dob ? new Date(user.dob).toLocaleDateString() : 'N/A'}</p></div>
                <div><p className="text-gray-500 dark:text-[var(--text-color)] mt-6">Gender</p><p>{user?.gender}</p></div>
                <div><p className="text-gray-500 dark:text-[var(--text-color)] mt-6">Time</p><p>{new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p></div>
                <div><p className="text-gray-500 dark:text-[var(--text-color)] mt-6">Location</p><p>California, USA</p></div>
              </div>
            </div>
          )}
        </div>
        ...
        <div className='w-full bg-gray-200  dark:bg-[var(--text-color)] rounded'>
          <div className="flex items-center justify-between cursor-pointer px-4 py-2.5 mt-3"
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-700 dark:text-[var(--text-color3)] font-medium">Privacy</span>
            </div>
            <div className='dark:text-[var(--text-color3)]'>
        ...
        <div className="w-full bg-gray-200 dark:bg-[var(--text-color)] rounded">
          <div className="flex items-center justify-between cursor-pointer px-4 py-2.5 mt-3.5"
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-700 dark:text-[var(--text-color3)] font-medium">Security</span>
            </div>
            <div className='dark:text-[var(--text-color3)]'>
