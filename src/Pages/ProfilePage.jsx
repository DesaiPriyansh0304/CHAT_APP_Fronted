...
function ProfilePage() {
  ...
  // console.log('theme --->/ProfilePage', theme);
      ...
      <div className='h-screen w-full dark:bg-[var(--)]'>
        ...
        <div className="p-2 relative">
          <div className="flex justify-between">
            <div className="p-5 text-2xl font-semibold dark:text-[var(--text-color3)]">My Profile</div>
            <div className="p-5 relative text-semibold">
              ...
              {isMenuOpen && (
                <div className="absolute right-5 top-12 w-40 bg-white dark:bg-[var(--text-color3)] border border-blue-300 rounded-md shadow-md z-10"
                  ref={dotmenuRef}>
                  {/*dot menu */}
                  {dotMenu.map(({ name, id }) => (
                    <div key={id}>
                      {name === "Another action" && <hr className="border-gray-300 dark:border-[var(--text-color)]  mx-2" />}
                      <button
          ...
          <div className='overflow-auto h-[50vh] mx-2'>
            ...
            <div className="w-full bg-gray-100 dark:bg-[var(--text-color)] rounded ">
              ...
              >
                <div className="flex items-center dark:text-[var(--text-color3)] gap-2 ">
                  <IoPersonOutline />
                  <span className="text-gray-700 dark:text-[var(--text-color3)]  font-medium">About</span>
                </div>
                {activeSection === "about" ? (
            ...
            <div className="w-full  bg-gray-100  dark:bg-[var(--text-color)] mt-2 rounded">
              ...
              >
                <div className="flex items-center dark:text-[var(--text-color3)] gap-2">
                  <MdAttachFile />
                  <span className="text-gray-700 dark:text-[var(--text-color3)] font-medium">Attached Files</span>
                </div>
              ...
              {activeSection === "attachfile" && (
                <div className='bg-white dark:bg-[var(--text-color3)]'>
                  <div className="mt-1 px-4 py-2.5 space-y-3 text-sm text-gray-800 max-h-[37vh] overflow-auto">
                    <div>
                      <p className="text-gray-500 dark:text-[var(--text-color)] mt-2">Uploaded by</p>
                      <p className="font-semibold">{user?.firstname} {user?.lastname}</p>
                    </div>
                    <div>
